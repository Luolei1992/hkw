...
function designXhr(designer) {
    ...
    for(var k = 0;k < designer.length;k++){
        ...
        totalDesigner += '<div class="ca-item ca-item-select ca-item-'+k+'" id="'+k+'" style=""><div class="ca-item-main"><div class="ca-item-photo"><a href="https://www.huakewang.com/2017_data/detailed.html?id='+designer[k].id+'" target=_blank><img alt="'+(designer[k].alt?designer[k].alt:"")+'" src="'+designer[k].path_thumb+'"/></a></div><h3 class="niCheng">'+designer[k].nick_name+(designer[k].sex == '男'?'<img class="sex_img_s" src="https://www.huakewang.com/2017_data/images/sex1.png">':(designer[k].sex=='女')?'<img class="sex_img_s" src="https://www.huakewang.com/2017_data/images/sex2.png">':"")+'</h3><p>'+(designer[k].jobs[0]?designer[k].jobs[0]:"设计师")+'<span class="shugang">|</span>'+_experience_text+'<span class="shugang">|</span>'+designer[k].works_count+'件作品 <br><span class="imgGZ1"></span><i class="text">'+designer[k].hits_count+'</i><span class="imgGZ2"></span><i class="text">'+designer[k].love_count+'</i><span class="imgGZ3"></span><i class="text">'+designer[k].comment_count+'</i></p><div class="ca-info"><div class="ca-star">'+getStart(designer[k].credit_val)+'</div><div class="ca-location"><span class="ca-location-icon"></span> '+get_adress(designer[k])+'</div></div><div class="ca-main"><div class="ca-inntro">'+goodSkills(designer[k].keywords)+'</div><div class="ca-list ca-list--thumb"><ul class="clearfix" style="height:352px;overflow: hidden;">'+proPics(designer[k])+'</ul></div></div><div class="ca-mian-detailed fn-hide"><div class="ca-mian-detailed-bottom fn-clear">'+(getCookie("user_id")?'<a class="talkOpacity" href="'+(designer[k].hxid?"https://www.huakewang.com/2017_data/Personal-x.html?talk_hxid="+designer[k].hxid:"javascript:;")+'" target="_blank"><div class="ca-mian-detailed-bottom-item fn-left"><i class="big-blue"></i>对话</div></a>':'<a class="talkOpacity" href="javascript:;" onclick="javascript:go_login();" target="_blank"><div class="ca-mian-detailed-bottom-item fn-left"><i class="big-blue"></i>对话</div></a>')+(getCookie("user_id")?'<a href="https://www.huakewang.com/2017_data/appointment.html?id='+designer[k].id+'" target=_blank><div class="ca-mian-detailed-bottom-item fn-left"><i class="big-red"></i>约见</div></a>':'<a href="javascript:;" onclick="javascript:go_login();"><div class="ca-mian-detailed-bottom-item fn-left"><i class="big-red"></i>约见</div></a>')+(getCookie("user_id")?'<a href="javascript:;" data-uid="'+designer[k].id+'" data-exprience="'+_experience_text+'" data-nickname="'+designer[k].nick_name+'" data-head="'+designer[k].path_thumb+'" data-works="'+designer[k].works_count+'" data-sex="'+designer[k].sex+'" data-address="'+designer[k].txt_address+'" class="big-orange-link"><div class="ca-mian-detailed-bottom-item fn-left"><i class="big-orange"></i>下单</div></a>':'<a href="javascript:;" onclick="javascript:go_login();"><div class="ca-mian-detailed-bottom-item fn-left"><i class="big-orange"></i>下单</div></a>')+'</div></div></div></div>';
    };
    if(totalDesigner == ""){
...
function goodSkills(numSkill) {
    ...
    for(var i = 0;i < numSkill.length;i++){
        if(numSkill[i] == "") {
            ...
            str += '<a href="https://www.huakewang.com/2017_data/designer.html?p='+numSkill[i]+'"><span class="sp1-bk ">'+numSkill[i]+'</span></a>';
        }
    }
    return str;
};
...
function proPics(pics) {
    ...
    if(!pics){
        ...
        for(var i = 0;i < 6;i++) {
            pic += '<li class="thumb h150"><a href="https://www.huakewang.com/2017_data/introduction.html?id='+pics.works_list[i].id+'" target="_blank"><img alt="'+(pics.works_list[i].alt?pics.works_list[i].alt:"")+'" src="https://www.huakewang.com/2017_data/images/1px.png" data-original="'+(pics.works_list[i].path.indexOf("upaiyun") != -1?pics.works_list[i].path+'!160x115':pics.works_list[i].path_thumb)+'"/></a></li>';
        }
        return pic;
    };
};
...
function worksPics(pics) {
    ...
    if(!pics){
        ...
        for(var i = 0;i < pics.length;i++) {
            pic += '<div><a href="https://www.huakewang.com/2017_data/introduction.html?id='+pics[i].id+'"><img style="width:118px;" src="https://www.huakewang.com/2017_data/images/1px.png" data-original="'+(pics[i].path.indexOf("upaiyun") != -1?pics[i].path+'!160x115':pics[i].path_thumb)+'" alt="'+(pics[i].alt?pics[i].alt:"")+'"></a></div>';
        };
        return pic;
    };
};
...
function workSkills(numSkill) {
    ...
    for(var i = 0;i < arr.length;i++){
        str += '<a href="https://www.huakewang.com/2017_data/designer.html?p='+arr[i]+'">'+arr[i]+'</a>'
    };
    return str;
};
...
function indexProXhr(projects,ele) {
    ...
    for(var k = 0;k < projects.length;k++){
        totalPro += '<div class="ca-item ca-item-select"><div class="ca-item-main"><div class="ca-item-photo"><a href="https://www.huakewang.com/2017_data/ProjectDetails.html?id='+projects[k].id+'" target=_blank><img alt="'+(projects[k].alt?projects[k].alt:"")+'" src="'+(projects[k].path?(projects[k].path.indexOf("upaiyun")!=-1?projects[k].path+'!138x138':projects[k].path_thumb):'https://www.huakewang.com/2017_data/images/selec.jpeg')+'"/></a></div><h3>找:'+getTitle(projects[k].title)+'</h3><p>'+projects[k].nick_name+'<span class="shugang">|</span>'+(projects[k].company?projects[k].company:"")+'<span class="shugang">|</span>'+(projects[k].job_name?projects[k].job_name:"")+'</p><p><em class="c-1">'+projects[k].budget_price+'</em></p><div class="ca-info"><div class="ca-18" style="font-size: 12px">'+getStart("5.0")+'</div><div class="ca-location"><span class="ca-location-icon"></span> '+get_adress(projects[k])+'</div></div><div class="ca-main"><div class="ca-inntro">'+getkeys(projects[k].keyword)+'</div><div class="ca-inntro c-6">'+projects[k].content+'</div></div><div class="ca-mian-detailed fn-hide"><div class="ca-mian-detailed-bottom fn-clear"><div class="ca-mian-detailed-bottom-item fl">'+(getCookie("user_id")?'<a class="talkOpacity" href="'+(projects[k].hxid?"https://www.huakewang.com/2017_data/Personal-x.html?talk_hxid="+projects[k].hxid:"javascript:;")+'" target="_blank"><i class="big-blue"></i>对话</a>':'<a class="talkOpacity" href="javascript:;" onclick="javascript:go_login();" target="_blank"><i class="big-blue"></i>对话</a>')+'</div>'+(getCookie("user_id")?'<a href="https://www.huakewang.com/2017_data/appointment.html?id='+projects[k].user_id+'&meet=0" target=_blank><div class="ca-mian-detailed-bottom-item fn-left"><i class="big-red"></i>约见</div></a>':'<a href="javascript:void(0);" onclick="javascript:go_login();" target=_blank><div class="ca-mian-detailed-bottom-item fn-left"><i class="big-red"></i>约见</div></a>')+(getCookie("user_id")?'<a href="javascript:;" target="_blank"><div class="ca-mian-detailed-bottom-item fn-left"><i class="big-orange" id="big-orange"></i>报价</div></a>':'<a href="javascript:void(0);" onclick="javascript:go_login();"><div class="ca-mian-detailed-bottom-item fn-left"><i class="big-orange" id="big-orange"></i>报价</div></a>')+'</div></div></div></div>';
    };
