...
$(function () {
        ...
        success: function (res) {
            if (res.success) {
                ...
                for (var i = 0; i < res.data.item_list.length; i++) {
                    ...
                    if (element.title) {
                        htmlStr += '<li><a target="_blank" href="newsDetails.html?id='+ element.id+'">▪ '+ element.title+'</a></li>';
                    }
                }
    ...
    var sort = "hits";
    //默认首页推荐为最多浏览
    worksList("NULL",sort,6,1,$('#indexWorkLis'));
    ...
    $(".hkw_link_pub").click(function () {
        ...
        if(sort == $(this).attr("data_val")){
            return false;
        }else{
            sort = $(this).attr("data_val");
            worksList("NULL",sort,6,1,$('#indexWorkLis'));
        }
    })
});
 ...
 * */
function worksList(kind,sort,num,n,ele) {
    $.ajax({
        url:baseURL+'get_works_list/'+kind+'/'+sort+'/'+num+'/'+n,
        type:"get",
        dataType:"json",
...
function indexWorkXhr(works,ele) {
    ...
    for(var k = 0;k < works.length;k++) {
        totalWorks += '<li class="cur"><div class="iInspir-block"></div><div class="hkw-work-img thumb-a h_a"><a href="introduction.html?id='+works[k].id+'" target=_blank><img class="scale" alt="'+(works[k].alt?works[k].alt:"")+'" src="images/1px.png" data-original="'+(works[k].path_thumb.indexOf("upaiyun") != -1?works[k].path+'!366x258':works[k].path_thumb)+'" width="366"/></a></div><div class="hkw-work-h3 hkw-view-h3"><h3>'+works[k].title+'</h3><div class="hkw-view"><h4>'+works[k].title+'</h4><div class="hkw-view-slide" style="overflow: hidden;"><div class="hkw-view-item"><span class="hkw-eye-icon"></span>'+works[k].hits+'</div> <div class="hkw-view-item"><span class="hkw-comment-icon"></span>'+works[k].comment_count+'</div><div class="hkw-view-item"><span class="hkw-collection-icon"></span>'+works[k].love_count+'</div></div><p title="'+works[k].abstract+'">'+works[k].abstract+'</p></div></div><div class="hkw-work-bottom"><a href="javscript:void(0);" target=_blank><img class="hkw-bottom-per" alt="'+(works[k].alt?works[k].alt:"")+'" src="'+(works[k].user_path?((works[k].user_path).indexOf("upaiyun") != -1?works[k].user_path+'!67x67':works[k].user_path_thumb):"selec.jpeg")+'" width="18" height="18"/><span class="hkw-join-name">'+works[k].nick_name+'</span></a><div class="hkw-work-bar fn-right"><div class="hkw-work-bar-item"><span class="hkw-join-icon"></span><span class="hkw-join-text">'+get_adress(works[k].user_info)+'</span><span class="hkw-join-text"> <span style="color: #e6e6e6;">|</span> '+works[k].user_info.experience+'</span></div></div><div class="hkw-person"><div class="hkw-person-t"><a href="detailed.html?id='+works[k].user_id+'"><img src="'+(works[k].user_path.indexOf("upaiyun") != -1?works[k].user_path+'!67x67':works[k].user_path_thumb)+'" alt=""><span>'+works[k].nick_name+'</span></a><p class="clearfix">'+workSkills(works[k].keyword)+'</p></div><div class="hkw-person-m">'+worksPics(works[k].user_info.userPageItemList)+'</div><div class="hkw-person-b"><div class="hkw-tell">'+(getCookie("user_id")?'<a class="talkOpacity" href="'+(works[k].user_info.hxid?"Personal-x.html?talk_hxid="+works[k].user_info.hxid:"javascript:;")+'" target="_blank"><i class="tell"><img src="./images/tell.png" alt=""></i><span>对话</span></a>':'<a class="talkOpacity" href="javascript:;" onclick="javascript:go_login()" target="_blank"><i class="tell"><img src="./images/tell.png" alt=""></i><span>对话</span></a>')+'</div><div class="hkw-jian">'+(getCookie("user_id")?'<a href="appointment.html?id='+works[k].user_id+'"><i class="jian"><img src="images/jian.png" alt=""></i><span>约见</span></a>':'<a href="javascript:;" onclick="javascript:go_login();"><i class="jian"><img src="images/jian.png" alt=""></i><span>约见</span></a>')+'</div><div class="hkw-xia">'+(getCookie("user_id")?'<a href="publish.html?uid='+works[k].user_id+'" target="_blank"><i class="xia"><img src="images/xia.png" alt=""></i><span>下单</span></a>':'<a href="javascript:;" onclick="javascript:go_login()"><i class="xia"><img src="images/xia.png" alt=""></i><span>下单</span></a>')+'</div></div></div></div></li>';
    };
...
function getCompanyLis(a) {
    ...
    for(var m = 0;m < a.length;m++){
        li += '<li class="companyPub"><a href="javascript:;" class="companyBrand companyBrand2"><img src="images/1px.png" data-original="'+a[m].path_thumb+'" alt="'+(a[m].alt?a[m].alt:"")+'" width="143" height="112"></a><div class="companyShow"><h3 class="companyTitle">'+a[m].title+'</h3><p>'+a[m].description+'</p><h3 class="companyTitle">'+a[m].company_des+'</h3><p><a href="'+a[m].html_path+'">'+a[m].html_path+'</a></div></li>';
    };
    return li;
};
