...
//日期转换
function a() {
    var mydate = ((new Date()).getTime() - 0) + (5 * 24 * 60 * 60 * 1000);
    return (new Date(parseInt(mydate)).toLocaleString().replace(/\/|\//g, "-").split(" ")[0]);
};
function b() {
    var mydates = ((new Date()).getTime() - 0) + (5 * 24 * 60 * 60 * 1000);
    var myarr = new Date(parseInt(mydates)).toLocaleString().split(" ")[0].split("/");
    return (myarr[0]+'年'+myarr[1]+'月'+myarr[2]+'日');
};
